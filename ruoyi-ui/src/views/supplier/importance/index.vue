<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="100px">
      ...
      <el-form-item label="采购金额占比" prop="percentage" style="width: 320px;">
        <el-input
          v-model="queryParams.percentage"
          placeholder="请输入采购金额占比"
          clearable
          @keyup.enter="handleQuery"
        />
      </el-form-item>
      <el-form-item label="得分" prop="score" style="width: 320px;">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['supplier:importance:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['supplier:importance:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['supplier:importance:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
              <el-button @click="handleImport" type="success" plain icon="Upload"
                         v-hasPermi="['supplier:importance:import']">导入
              </el-button>
            </el-col>
    ...
    <el-table v-loading="loading" :data="importanceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['supplier:importance:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['supplier:importance:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="importanceRef" :model="form" :rules="rules" label-width="100px">
...
const data = reactive({
  ...
  rules: {
    ...
    amount:[
      { required: true, message: "供货金额不能为空", trigger: "blur" }
    ],
    uploadTime:[
...
function getList() {
  loading.value = true;
  listImportance(queryParams.value).then(response => {
    importanceList.value = response.rows;
...
function handleUpdate(row) {
  ...
  getImportance(_id).then(response => {
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["importanceRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateImportance(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addImportance(form.value).then(response => {
...
function handleExport() {
  proxy.download('supplier/importance/export', {
    ...queryParams.value
  }, `importance_${new Date().getTime()}.xlsx`)
}
