    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['security:uploadfile:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['security:uploadfile:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['security:uploadfile:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['security:uploadfile:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="uploadfileList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['security:uploadfile:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['security:uploadfile:remove']">删除</el-button>
...
import { listUploadfile, getUploadfile, delUploadfile, addUploadfile, updateUploadfile } from "@/api/security/uploadfile";
...
function getList() {
  loading.value = true;
  listUploadfile(queryParams.value).then(response => {
...
function handleUpdate(row) {
  ...
  getUploadfile(_id).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除文件预览编号为"' + _ids + '"的数据项？').then(function() {
    return delUploadfile(_ids);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('security/uploadfile/export', {
    ...queryParams.value
  }, `uploadfile_${new Date().getTime()}.xlsx`);
}
