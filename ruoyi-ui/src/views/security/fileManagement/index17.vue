...
const upload = reactive({
  ...
  // 上传的地址 - 使用应急演练计划的导入接口，通过sourceUrl参数指定来源为文件管理模块
  url: import.meta.env.VITE_APP_BASE_API + "/security/emergencydrillplan/importData?sourceUrl=" + encodeURIComponent("securityConm/security1/run/Emergency-Preparedness-Response/emergencydrillplan")
});
...
function resetQuery() {
  ...
  queryParams.value.moduleName = "应急演练计划";
  handleQuery();
}
...
async function submitUpload() {
  ...
  try {
    ...
    for (let i = 0; i < totalFiles; i++) {
      ...
      formData.append("sourceUrl", "securityConm/security1/run/Emergency-Preparedness-Response/emergencydrillplan");
