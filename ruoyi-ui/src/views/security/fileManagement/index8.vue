<template>
  <div class="app-container">
   ...
   <!-- 多文件上传区域 -->
    ...
    <!-- 导入对话框 - 复用planning/index5.vue的导入功能 -->
...
// 复用planning/index5.vue的导入API
import { downloadTemplate } from "@/api/security/RegulationsIdentificationList1";
...
// 导入功能相关变量 - 复用planning/index5.vue的导入方法
...
function resetQuery() {
  ...
  queryParams.value.moduleName = "安全法律法规识别清单";
  handleQuery();
}
...
/** 下载模板按钮操作 - 复用planning/index5.vue的模板下载方法 */
...
/** 触发自定义导入对话框 - 复用planning/index5.vue的导入对话框 */
function handleCustomImport() {
  importDialogVisible.value = true;
  selectedFile.value = null;
}

/** 触发文件选择 - 复用planning/index5.vue的文件选择方法 */
function triggerFileSelect() {
  fileInput.value.click();
}

// /** 处理文件选择变更 - 复用planning/index5.vue的文件选择处理 */
...
/** 重置文件输入框 - 复用planning/index5.vue的文件重置方法 */
...
/** 重置导入对话框 - 复用planning/index5.vue的对话框重置方法 */
function resetImportDialog() {
  resetFileInput();
  importLoading.value = false;
}

/** 提交导入文件 - 复用planning/index5.vue的文件导入方法，添加sourceUrl参数 */
function submitImportFile() {
  ...
  // 添加sourceUrl参数传递给后端
  formData.append('sourceUrl', 'securityConm/security1/planning/legal/legal');
