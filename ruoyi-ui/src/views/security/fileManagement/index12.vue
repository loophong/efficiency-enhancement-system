<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="考试名称" prop="text">
        <el-input
 ...
 <!-- 多文件上传区域 -->
    ...
    <el-table v-loading="loading" :data="filemanagementList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="考试名称" align="center" prop="text" />
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="filemanagementRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="考试名称" prop="text">
          <el-input v-model="form.text" type="textarea" placeholder="请输入考试名称" />
        </el-form-item>
    ...
    <!-- 导入对话框 - 复用planning/index8.vue的导入方法 -->
...
/*** 用户导入参数 - 复用planning/index8.vue的导入方法 */
const upload = reactive({
  ...
  // 添加sourceUrl参数传递给后端
  data: {
    sourceUrl: 'securityConm/security1/support/tranning/knowledgeassessment'
  }
});
...
function resetQuery() {
  ...
  queryParams.value.moduleName = "安全知识考核";
  handleQuery();
}
...
/** 导入按钮操作 - 复用planning/index8.vue的导入方法 */
function handleImport() {
  upload.title = "安全知识考核导入";
  upload.open = true;
}

/** 下载模板操作 - 复用planning/index8.vue的模板下载方法 */
function handleDownloadTemplate() {
  proxy.download('security/knowledgeassessment/importTemplate', {}, `安全知识考核模板_${new Date().getTime()}.xlsx`);
}

/** 文件上传中处理 - 复用planning/index8.vue的上传进度处理 */
function handleFileUploadProgress(event, file, fileList) {
  upload.isUploading = true;
}

/** 文件上传成功处理 - 复用planning/index8.vue的上传成功处理 */
...
/** 提交上传文件 - 复用planning/index8.vue的文件提交方法 */
...
async function submitUpload() {
  ...
  try {
    ...
    for (let i = 0; i < totalFiles; i++) {
      ...
      formData.append("sourceUrl", "securityConm/security1/support/tranning/knowledgeassessment");
