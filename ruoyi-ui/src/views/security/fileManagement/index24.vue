...
// 导入功能相关变量 - 复用planning/index00.vue的导入方法
...
function resetQuery() {
  proxy.resetForm("queryRef");
   queryParams.value.moduleName = "环保目标指标考核达成";
  handleQuery();
}
...
// 复用planning/index00.vue的导入相关函数

/** 导入按钮操作 - 复用planning/index00.vue的导入方法 */
function handleImport() {
  uploadDialogVisible.value = true;
  selectedFile.value = null;
}

/** 下载模板按钮操作 - 复用planning/index00.vue的模板下载方法 */
function handleDownloadTemplate() {
  ...
  try {
    ...
    // 使用原生的下载方法
    proxy.download('security/HealthAndSafetyGoals/importTemplate', {}, `环保目标指标考核表导入模板_${new Date().getTime()}.xlsx`, 'get');
    console.log('下载请求已发送');
  } catch (error) {
...
/** 上传文件超出数量限制 - 复用planning/index00.vue */
function handleExceed() {
  proxy.$modal.msgError("最多只能上传一个文件！");
}

/** 文件选择变更 - 复用planning/index00.vue */
...
/** 文件移除前的处理 - 复用planning/index00.vue */
function handleBeforeRemove() {
  selectedFile.value = null;
  return true;
}

/** 取消上传 - 复用planning/index00.vue */
...
/** excel文件上传 - 复用planning/index00.vue的上传方法，但调用文件管理相关的API */
function uploadFile() {
  try {
    ...
    formData.append('sourceUrl', 'securityConm/security1/planning/indication/annualcontrol2');
    ...
    // 复用planning/index00.vue的导入API，但通过sourceUrl区分调用来源
