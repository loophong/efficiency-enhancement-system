  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['security:annualplan:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['security:annualplan:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['security:annualplan:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['security:annualplan:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="annualplanList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['security:annualplan:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['security:annualplan:remove']">删除</el-button>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="annualplanRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="上传时间" prop="uploadDate">
          <el-date-picker clearable
...
function getList() {
  loading.value = true;
  listAnnualplan(queryParams.value).then(response => {
    annualplanList.value = response.rows;
...
function handleUpdate(row) {
  ...
  getAnnualplan(_id).then(response => {
    form.value = response.data;
    open.value = true;
    title.value = "修改环境和职业健康安全方针";
  });
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["annualplanRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateAnnualplan(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addAnnualplan(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除环境和职业健康安全方针编号为"' + _ids + '"的数据项？').then(function() {
    return delAnnualplan(_ids);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('security/annualplan/export', {
    ...queryParams.value
  }, `annualplan_${new Date().getTime()}.xlsx`)
}

/** 预览文件 */
function handlePreview(row) {
  // 检查文件路径是否存在 (注意：字段名是 files 不是 file)
  console.log("预览文件 - 行数据:", row);
  console.log("文件字段值:", row.files);

  if (!row.files || typeof row.files !== 'string' || row.files.trim() === '') {
    proxy.$modal.msgError("文件路径不存在或格式错误");
