<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['security:copy222:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['security:copy222:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['security:copy222:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['security:copy222:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="copy222List" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="copy222Ref" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="上传时间" prop="uploadDate">
          <el-date-picker clearable
...
function getList() {
  loading.value = true;
  listCopy222(queryParams.value).then(response => {
    copy222List.value = response.rows;
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["copy222Ref"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateCopy222(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addCopy222(form.value).then(response => {
...
function handleExport() {
  proxy.download('security/copy222/export', {
    ...queryParams.value
