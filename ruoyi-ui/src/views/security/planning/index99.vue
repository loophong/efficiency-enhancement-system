<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['security:compliancereport:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['security:compliancereport:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['security:compliancereport:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['security:compliancereport:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="compliancereportList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['security:compliancereport:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['security:compliancereport:remove']">删除</el-button>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="compliancereportRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="上传时间" prop="uploadDate">
          <el-date-picker clearable
...
<script setup name="Compliancereport">
import VueOfficeDocx from '@vue-office/docx'
import { listCompliancereport, getCompliancereport, delCompliancereport, addCompliancereport, updateCompliancereport } from "@/api/security/compliancereport";
...
const compliancereportList = ref([]);
...
function getList() {
  loading.value = true;
  listCompliancereport(queryParams.value).then(response => {
    compliancereportList.value = response.rows;
...
function reset() {
  ...
  proxy.resetForm("compliancereportRef");
}
...
function handleUpdate(row) {
  ...
  getCompliancereport(_id).then(response => {
    form.value = response.data;
    open.value = true;
    title.value = "修改合规性评价报告";
  });
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["compliancereportRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateCompliancereport(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addCompliancereport(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除合规性评价报告编号为"' + _ids + '"的数据项？').then(function() {
    return delCompliancereport(_ids);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('security/compliancereport/export', {
    ...queryParams.value
  }, `compliancereport_${new Date().getTime()}.xlsx`)
}
