<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
            ...
            icon="Plus"
            @click="handleAdd"
            v-hasPermi="['security:ValveTestReport:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
            ...
            :disabled="single"
            @click="handleUpdate"
            v-hasPermi="['security:ValveTestReport:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
            ...
            :disabled="multiple"
            @click="handleDelete"
            v-hasPermi="['security:ValveTestReport:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
            ...
            icon="Download"
            @click="handleExport"
            v-hasPermi="['security:ValveTestReport:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="ValveTestReportList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['security:ValveTestReport:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['security:ValveTestReport:remove']">删除</el-button>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="ValveTestReportRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="上传时间" prop="uploadDate">
          <el-date-picker clearable
...
<script setup name="ValveTestReport">
import { listValveTestReport, getValveTestReport, delValveTestReport, addValveTestReport, updateValveTestReport } from "@/api/security/ValveTestReport";
...
function getList() {
  loading.value = true;
  listValveTestReport(queryParams.value).then(response => {
    ValveTestReportList.value = response.rows;
...
function reset() {
  ...
  proxy.resetForm("ValveTestReportRef");
}
...
function handleUpdate(row) {
  ...
  getValveTestReport(_id).then(response => {
    form.value = response.data;
    open.value = true;
    title.value = "修改安全阀检测报告";
  });
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["ValveTestReportRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateValveTestReport(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addValveTestReport(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除安全阀检测报告编号为"' + _ids + '"的数据项？').then(function() {
    return delValveTestReport(_ids);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('security/ValveTestReport/export', {
    ...queryParams.value
  }, `ValveTestReport_${new Date().getTime()}.xlsx`)
}
