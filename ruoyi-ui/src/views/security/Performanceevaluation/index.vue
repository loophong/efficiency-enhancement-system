<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
            ...
            icon="Plus"
            @click="handleAdd"
            v-hasPermi="['security:WorkPointInspectionReport:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
            ...
            :disabled="single"
            @click="handleUpdate"
            v-hasPermi="['security:WorkPointInspectionReport:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
            ...
            :disabled="multiple"
            @click="handleDelete"
            v-hasPermi="['security:WorkPointInspectionReport:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
            ...
            icon="Download"
            @click="handleExport"
            v-hasPermi="['security:WorkPointInspectionReport:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="WorkPointInspectionReportList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="WorkPointInspectionReportRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="上传时间" prop="uploadDate">
          <el-date-picker clearable
...
function getList() {
  loading.value = true;
  listWorkPointInspectionReport(queryParams.value).then(response => {
    WorkPointInspectionReportList.value = response.rows;
...
function reset() {
  ...
  proxy.resetForm("WorkPointInspectionReportRef");
}
...
function handleUpdate(row) {
  ...
  getWorkPointInspectionReport(_id).then(response => {
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["WorkPointInspectionReportRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateWorkPointInspectionReport(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addWorkPointInspectionReport(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除职业危害作业点检测报告编号为"' + _ids + '"的数据项？').then(function() {
    return delWorkPointInspectionReport(_ids);
  }).then(() => {
    getList();
