<template>
      ...
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
          <el-button
            ...
            icon="Plus"
            @click="handleAdd"
            v-hasPermi="['security:SpecialCheck:add']"
          >新增</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            :disabled="single"
            @click="handleUpdate"
            v-hasPermi="['security:SpecialCheck:edit']"
          >修改</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            :disabled="multiple"
            @click="handleDelete"
            v-hasPermi="['security:SpecialCheck:remove']"
          >删除</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            icon="Download"
            @click="handleExport"
            v-hasPermi="['security:SpecialCheck:export']"
          >导出</el-button>
        </el-col>
        <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
      </el-row>
  
      <el-table v-loading="loading" :data="SpecialCheckList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['security:SpecialCheck:edit']">修改</el-button>
            <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['security:SpecialCheck:remove']">删除</el-button>
    ...
    <el-dialog title="文件预览" v-model="previewDialogVisible" width="80%" append-to-body>
      ...
      <el-dialog :title="title" v-model="open" width="500px" append-to-body>
        <el-form ref="SpecialCheckRef" :model="form" :rules="rules" label-width="80px">
          <el-form-item label="上传时间" prop="uploadDate">
            <el-date-picker clearable
  ...
  <script setup name="SpecialCheck">
  import { listSpecialCheck, getSpecialCheck, delSpecialCheck, addSpecialCheck, updateSpecialCheck } from "@/api/security/SpecialCheck";
  ...
  function getList() {
    loading.value = true;
    listSpecialCheck(queryParams.value).then(response => {
      SpecialCheckList.value = response.rows;
...
function handlePreview(row) {
   const staticPath = import.meta.env.VITE_APP_BASE_API ; 
...
function errorHandler(error) {
  ...
  function reset() {
    ...
    proxy.resetForm("SpecialCheckRef");
  }
  ...
  function handleUpdate(row) {
    ...
    getSpecialCheck(_id).then(response => {
      form.value = response.data;
      open.value = true;
      title.value = "修改专项检查记录";
    });
  }
  
  /** 提交按钮 */
  function submitForm() {
    proxy.$refs["SpecialCheckRef"].validate(valid => {
      if (valid) {
        if (form.value.id != null) {
          updateSpecialCheck(form.value).then(response => {
            proxy.$modal.msgSuccess("修改成功");
            open.value = false;
            getList();
          });
        } else {
          addSpecialCheck(form.value).then(response => {
  ...
  function handleDelete(row) {
    ...
    proxy.$modal.confirm('是否确认删除专项检查记录编号为"' + _ids + '"的数据项？').then(function() {
      return delSpecialCheck(_ids);
    }).then(() => {
      getList();
  ...
  function handleExport() {
    proxy.download('security/SpecialCheck/export', {
      ...queryParams.value
    }, `SpecialCheck_${new Date().getTime()}.xlsx`)
  }
