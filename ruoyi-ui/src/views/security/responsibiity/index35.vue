<template>
    <div class="app-container">
      ...
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
          <el-button
            ...
            icon="Plus"
            @click="handleAdd"
            v-hasPermi="['security:SeasonList:add']"
          >新增</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            :disabled="single"
            @click="handleUpdate"
            v-hasPermi="['security:SeasonList:edit']"
          >修改</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            :disabled="multiple"
            @click="handleDelete"
            v-hasPermi="['security:SeasonList:remove']"
          >删除</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            icon="Download"
            @click="handleExport"
            v-hasPermi="['security:SeasonList:export']"
          >导出</el-button>
        </el-col>
      ...
      <el-table v-loading="loading" :data="SeasonListList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['security:SeasonList:edit']">修改</el-button>
            <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['security:SeasonList:remove']">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-dialog :title="title" v-model="open" width="500px" append-to-body>
        <el-form ref="SeasonListRef" :model="form" :rules="rules" label-width="80px">
          <el-form-item label="上传时间" prop="uploadDate">
            <el-date-picker clearable
  ...
  function getList() {
    loading.value = true;
    listSeasonList(queryParams.value).then(response => {
      SeasonListList.value = response.rows;
  ...
  function handleUpdate(row) {
    ...
    getSeasonList(_id).then(response => {
      form.value = response.data;
      open.value = true;
      title.value = "修改季节性检查记录";
    });
  }
  
  /** 提交按钮 */
  function submitForm() {
    proxy.$refs["SeasonListRef"].validate(valid => {
      if (valid) {
        if (form.value.id != null) {
          updateSeasonList(form.value).then(response => {
            proxy.$modal.msgSuccess("修改成功");
            open.value = false;
            getList();
          });
        } else {
          addSeasonList(form.value).then(response => {
  ...
  function handleDelete(row) {
    ...
    proxy.$modal.confirm('是否确认删除季节性检查记录编号为"' + _ids + '"的数据项？').then(function() {
      return delSeasonList(_ids);
    }).then(() => {
      getList();
  ...
  function handleExport() {
    proxy.download('security/SeasonList/export', {
      ...queryParams.value
    }, `SeasonList_${new Date().getTime()}.xlsx`)
  }
