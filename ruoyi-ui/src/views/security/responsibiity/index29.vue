<template>
    <div class="app-container">
      ...
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
          <el-button
            ...
            icon="Plus"
            @click="handleAdd"
            v-hasPermi="['security:RelationList1:add']"
          >新增</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            :disabled="single"
            @click="handleUpdate"
            v-hasPermi="['security:RelationList1:edit']"
          >修改</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            :disabled="multiple"
            @click="handleDelete"
            v-hasPermi="['security:RelationList1:remove']"
          >删除</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            icon="Download"
            @click="handleExport"
            v-hasPermi="['security:RelationList1:export']"
          >导出</el-button>
        </el-col>
      ...
      <el-dialog :title="title" v-model="open" width="500px" append-to-body>
        <el-form ref="RelationList1Ref" :model="form" :rules="rules" label-width="80px">
          <el-form-item label="上传时间" prop="uploadDate">
            <el-date-picker clearable
  ...
  function getList() {
    loading.value = true;
    listRelationList1(queryParams.value).then(response => {
      RelationList1List.value = response.rows;
  ...
  function reset() {
    ...
    proxy.resetForm("RelationList1Ref");
  }
  ...
  function handleUpdate(row) {
    ...
    getRelationList1(_id).then(response => {
    ...
    proxy.$refs["RelationList1Ref"].validate(valid => {
      if (valid) {
        if (form.value.id != null) {
          updateRelationList1(form.value).then(response => {
            proxy.$modal.msgSuccess("修改成功");
            open.value = false;
            getList();
          });
        } else {
          addRelationList1(form.value).then(response => {
  ...
  function handleDelete(row) {
    ...
    proxy.$modal.confirm('是否确认删除相关方检查记录编号为"' + _ids + '"的数据项？').then(function() {
      return delRelationList1(_ids);
    }).then(() => {
      getList();
  ...
  function handleExport() {
    proxy.download('security/RelationList1/export', {
      ...queryParams.value
    }, `RelationList1_${new Date().getTime()}.xlsx`)
  }
