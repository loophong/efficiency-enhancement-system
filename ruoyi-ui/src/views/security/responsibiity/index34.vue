<template>
      ...
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
          <el-button
            ...
            icon="Plus"
            @click="handleAdd"
            v-hasPermi="['security:ImportantList:add']"
          >新增</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            :disabled="single"
            @click="handleUpdate"
            v-hasPermi="['security:ImportantList:edit']"
          >修改</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            :disabled="multiple"
            @click="handleDelete"
            v-hasPermi="['security:ImportantList:remove']"
          >删除</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
            ...
            icon="Download"
            @click="handleExport"
            v-hasPermi="['security:ImportantList:export']"
          >导出</el-button>
        </el-col>
        <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
      </el-row>
  
      <el-table v-loading="loading" :data="ImportantListList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['security:ImportantList:edit']">修改</el-button>
            <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['security:ImportantList:remove']">删除</el-button>
    ...
    <el-dialog title="文件预览" v-model="previewDialogVisible" width="80%" append-to-body>
      ...
      <el-dialog :title="title" v-model="open" width="500px" append-to-body>
        <el-form ref="ImportantListRef" :model="form" :rules="rules" label-width="80px">
          <el-form-item label="上传时间" prop="uploadDate">
            <el-date-picker clearable
  ...
  <script setup name="ImportantList">
  import { listImportantList, getImportantList, delImportantList, addImportantList, updateImportantList } from "@/api/security/ImportantList";
...
function errorHandler(error) {
 ...
 /** 查询重点部位检查记录列表 */
  function getList() {
    loading.value = true;
    listImportantList(queryParams.value).then(response => {
      ImportantListList.value = response.rows;
  ...
  function reset() {
    ...
    proxy.resetForm("ImportantListRef");
  }
  ...
  function handleUpdate(row) {
    ...
    getImportantList(_id).then(response => {
      form.value = response.data;
      open.value = true;
      title.value = "修改重点部位检查记录";
    });
  }
  
  /** 提交按钮 */
  function submitForm() {
    proxy.$refs["ImportantListRef"].validate(valid => {
      if (valid) {
        if (form.value.id != null) {
          updateImportantList(form.value).then(response => {
            proxy.$modal.msgSuccess("修改成功");
            open.value = false;
            getList();
          });
        } else {
          addImportantList(form.value).then(response => {
  ...
  function handleDelete(row) {
    ...
    proxy.$modal.confirm('是否确认删除重点部位检查记录编号为"' + _ids + '"的数据项？').then(function() {
      return delImportantList(_ids);
    }).then(() => {
      getList();
  ...
  function handleExport() {
    proxy.download('security/ImportantList/export', {
      ...queryParams.value
    }, `ImportantList_${new Date().getTime()}.xlsx`)
  }
