...
import { listKnowledgeassessment, listKnowledgeassessmentByRelatedId, getKnowledgeassessment, delKnowledgeassessment, addKnowledgeassessment, updateKnowledgeassessment } from "@/api/security/knowledgeassessment";
...
function getList() {
  ...
  if (queryParams.value.relatedId) {
    listKnowledgeassessmentByRelatedId(queryParams.value.relatedId, route.query.sourceModule).then(response => {
...
function importTemplate() {
  proxy.download('security/knowledgeassessment/importTemplate', {}, `安全知识考核导入模板_${new Date().getTime()}.xlsx`);
}
