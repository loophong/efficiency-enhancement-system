<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['security:fileList:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['security:fileList:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['security:fileList:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['security:fileList:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="fileListList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['security:fileList:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['security:fileList:remove']">删除</el-button>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="fileListRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="文件名称" prop="fileName">
...
import { listFileList, getFileList, delFileList, addFileList, updateFileList } from "@/api/security/fileList";
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["fileListRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        ...
        addFileList(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除制度明细编号为"' + _ids + '"的数据项？').then(function() {
    return delFileList(_ids);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('security/fileList/export', {
    ...queryParams.value
